...
- [Rust Study](#rust-study)
  ...
  - [语法](#语法)
    ...
    - [内存管理](#内存管理)
      - [Box 智能指针](#box-智能指针)
      - [Rc 引用计数智能指针](#rc-引用计数智能指针)
      - [RefCell Cell](#refcell-cell)
      - [弱引用](#弱引用)
    - [trait 对象](#trait-对象)
    - [模式匹配](#模式匹配)
    - [unsafe](#unsafe)
    - [高级 trait、类型](#高级-trait类型)
    - [高级函数](#高级函数)
    - [宏](#宏)
    - [并发](#并发)
    - [异步](#异步)
...
- Dioxus-CLI: `cargo install --git https://github.com/DioxusLabs/cli`
- cargo-edit `cargo install cargo-edit`
- 创建 rust 项目
- 处理桌面端的依赖 `cargo add dioxus --features desktop`
...
- 元素属性直接用 `key: "value",` 形式声明，减号用下划线代替
- 必须放在内容和子元素之前
- 如果元素属性的 key 不是预设的，需要加双引号
- hidden, disable 等属性是布尔属性，可以填 "true" / "false"，false 时属性将被忽略
...
```rs
div {
    onclick: move |_| { dbg!("Div 被点击了！"); },
    "xxx"
}
```

拦截默认事件：加一个属性 `prevent_default: "onclick"`
...
`Element` 类型本质上是 `Option<VNode>`，可以用 `None` 代表空元素
...
let name_list = names.iter().map(|name| {
    rsx!(
        li {
            key: "{name}",
            "{name}"
...
当 struct 没有借用时必须实现 PartialEq trait（不是很懂）

该函数似乎必须大写开头，加 `#[allow(non_snake_case)]` 来消除 warning
...
可以用 `dioxus::desktop::launch_with_props(App, props)` 来允许 App 接受 props

用 Hook 更新 State

- 用常规方法整个替换 State 将触发所有内容的渲染
- 用 `use_state` 来更新状态（在双向绑定里貌似搞过）
- 用 `use_future` 来完成异步的数据更新

use_state

- `state.setter()` 得到 `Rc<dyn Fn(T)>` 类型，父组件向子组件可以传递该值

`cx.use_hook` 返回数据的可变引用（似乎完全没用）

- 要求定义时只能顺序地定义，不能用条件语句
- 可变引用只能被移动一次，因此用 Cell / RefCell 的不可变引用完成传递多次的操作（完全没搞明白怎么搞）

```rs
let name: &mut String = cx.use_hook(|| "John Doe".to_string());
let name2: &RefCell<String> = cx.use_hook(|_| RefCell::new(String::from("John Doe")));
...
win.with_inner_size(Size::Logical(
    LogicalSize {
        width: 1000.0,
        height: 500.0,
    },
...
`#![windows_subsystem = "windows"]` 隐藏命令行窗口
